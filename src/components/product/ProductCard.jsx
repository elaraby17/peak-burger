    ...
    >
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-cream-100">
        ...
        <button
          ...
          className="absolute end-3 top-3 flex h-9 w-9 items-center justify-center rounded-full bg-white/90 text-ink shadow-card backdrop-blur transition-transform active:scale-90"
        >
          <Heart className={cn("h-4.5 w-4.5", favorite ? "fill-secondary text-secondary" : "text-ink-soft")} />
        </button>
      </div>

      <div className="flex flex-1 flex-col gap-1 p-4">
        <h3 className="font-display text-base font-bold text-ink line-clamp-1">{t(product.name)}</h3>
        <p className="line-clamp-2 flex-1 text-sm text-ink-soft">{t(product.description)}</p>
        <div className="mt-3 flex items-center justify-between">
          <Price value={displayPrice} from={hasSizes} className="text-ink" />
          <button
            onClick={handleQuickAdd}
            aria-label="Add to cart"
            className="flex h-10 w-10 items-center justify-center rounded-full bg-secondary text-white shadow-pop transition-transform active:scale-90 group-hover:scale-105"
          >
