...
export default function AdminBranches() {
  ...
  const columns = [
    { key: "name", header: lang === "ar" ? "الاسم" : "Name", render: (b) => <span className="font-semibold text-text">{lang === "ar" ? b.nameAr : b.nameEn}</span> },
    ...
    { key: "actions", header: "", render: (b) => (
      <div className="flex gap-1">
        <button onClick={() => setEditing(b)} className="flex h-8 w-8 items-center justify-center rounded-full text-text-muted hover:bg-surface-hover"><Pencil className="h-4 w-4" /></button>
        <button onClick={() => handleToggle(b)} className="flex h-8 w-8 items-center justify-center rounded-full text-text-muted hover:bg-surface-hover"><Power className="h-4 w-4" /></button>
        <button onClick={() => handleDelete(b)} className="flex h-8 w-8 items-center justify-center rounded-full text-secondary hover:bg-secondary/15"><Trash2 className="h-4 w-4" /></button>
      </div>
    ) },
  ];
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="font-display text-2xl font-extrabold text-text">{lang === "ar" ? "الفروع" : "Branches"}</h1>
