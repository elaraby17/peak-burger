...
export default function AdminReviews() {
  ...
  const handleDelete = async (r) => {
    const confirmed = await confirmDialog({
      title: lang === "ar" ? "حذف التقييم؟" : "Delete this review?",
  ...
  const columns = [
    {
      key: "customer",
      header: lang === "ar" ? "العميل" : "Customer",
      render: (r) => (
...

    {
      ...
      render: (r) => (
        <span className="line-clamp-1 max-w-xs text-ink-soft">
          {r.comment}
        </span>
      ),
    },
      ...
      render: (r) => (
        <div className="flex gap-1">
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-full text-secondary hover:bg-secondary-50"
            title="Approve"
          >
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable
        columns={columns}
        rows={reviews}
        emptyTitle={lang === "ar" ? "مفيش تقييمات" : "No reviews"}
      />
    </div>
  );
}